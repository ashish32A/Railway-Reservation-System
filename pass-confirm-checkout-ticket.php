<?php
    ...
    if(isset($_POST['train_fare_confirm_checkout']))
    {
            ...
            $query="insert into orrs_train_tickets (pass_name, pass_addr, pass_email, train_name, train_no, train_dep_stat, train_arr_stat,  train_fare,confirmation) values (?,?,?,?,?,?,?,?,?)";
            $stmt = $mysqli->prepare($query); //prepare sql and bind it later
            $rc=$stmt->bind_param('sssssssss', $pass_name, $pass_addr, $pass_email, $train_name, $train_no, $train_dep_stat, $train_arr_stat, $train_fare,$confirm);
            $stmt->execute();
            if($stmt)
            {

<?php
    ...
    if(isset($_POST['Book_Train']))
    {
            ...
            */
            ...
            $pass_train_fare = $_POST['orderAmount'];
            $seat=rand(1,100);
            $coah=rand(1,18);
            
            //sql file to update the table of passengers with the new captured information
            $query="update  orrs_passenger set pass_train_number = ?, pass_train_name = ?, pass_dep_station = ?, pass_dep_time = ?,  pass_arr_station = ?, pass_train_fare = ? ,coach_no = ? ,seat_no = ? where pass_id=?";
            $stmt = $mysqli->prepare($query); //prepare sql and bind it later
            $rc=$stmt->bind_param('ssssssiii', $pass_train_number, $pass_train_name, $pass_dep_station, $pass_dep_time, $pass_arr_station, $pass_train_fare,$coah,$seat ,$aid);
            $stmt->execute();
            if($stmt)
            {
              ...
              <div class="card card-border-color card-border-color-success">
                ...
                <div class="card-body">
                  <form method ="POST" action="request.php">
                    ...
                    <input type="hidden" class="form-control" name="appId" value="<?php echo APPID ?>"/>
                <input type="hidden" class="form-control" name="orderCurrency" value="<?php echo CURRENCY ?>" />
                <input type="hidden" class="form-control" name="returnUrl" value="<?php echo RETURNURL ?>"/>
                <input type="hidden" class="form-control" name="notifyUrl" value="<?php echo NOTIFYURL ?>"/>
                
                      <label class="col-12 col-sm-3 col-form-label text-sm-right" for="inputText3"> Full Name</label>
                      <div class="col-12 col-sm-8 col-lg-6">
                        <input class="form-control"  name="customerName" value="<?php echo $row->pass_fname."  ".$row->pass_lname;?>" id="inputText3" type="text">
                      </div>
                    </div>
                    ...
                    <div class="form-group row">
                      ...
                      <div class="col-12 col-sm-8 col-lg-6">
                        <input class="form-control"  name="customerPhone" value="<?php echo $row->pass_phone;?>" id="inputText3" type="text">
                      </div>
                    </div>
                    <div class="form-group row">
                      <label class="col-12 col-sm-3 col-form-label text-sm-right" for="inputText3">Email</label>
                      <div class="col-12 col-sm-8 col-lg-6">
                        <input class="form-control"  name="customerEmail" value="<?php echo $row->pass_email;?>" id="inputText3" type="text">
                      </div>
                    </div>
                    <div class="form-group row">
                    ...
                    <div class="form-group row">
                      ...
                      <div class="col-12 col-sm-8 col-lg-6">
                        <input class="form-control" readonly name="orderAmount" value="<?php echo $row->fare;?>"  id="inputText3" type="text">
                      </div>
                    </div>
                    <!--End TRain  isntance-->
                    <?php }?>
